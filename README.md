...
PitaMado は、ブラウザで調べながらターミナルでAIやコードに指示を出し、Finderでファイルや画像を確認する――そんなmacの作業画面を、ワンクリックで使いやすく整えるアプリです。

Chrome系ブラウザ、Finder、ターミナルなど、いま使いたいウインドウを見やすい位置と大きさに配置します。画面を切り替えたり、毎回ウインドウのサイズを調整したりする手間を減らせます。

Dockには表示されない軽量なメニューバーアプリで、macOSのアクセシビリティAPIを使って通常のアプリウインドウを移動・リサイズします。
